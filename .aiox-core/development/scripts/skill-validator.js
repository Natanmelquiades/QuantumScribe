...
class SkillValidator {
  ...
  validateDependencies(deps, result) {
    ...
    for (const [type, items] of Object.entries(deps)) {
      if (!validTypes.includes(type) && !nonFileTypes.includes(type)) {
  ...
  async validateTask(filePath) {
    ...
    try {
      ...
      // Check for YAML frontmatter (optional). Many legacy tasks begin with
      // a horizontal-rule separator before prose; only parse it as
      // frontmatter when the second line contains YAML content.
      const taskHeaderLines = content.split(/\r?\n/, 3);
      if (taskHeaderLines[0]?.trim() === '---' && taskHeaderLines[1]?.trim()) {
