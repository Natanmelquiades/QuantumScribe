...
class WorkflowValidator {
  ...
  validateRequiredFields(workflow, filePath) {
    ...
    const isEpicOrchestrationTemplate =
      wf.type === 'epic-orchestration' &&
      wf.inner_workflow &&
      wf.wave_pattern;

    if (!hasSequence && !hasPhases && isEpicOrchestrationTemplate) {
      result.suggestions.push(
        'Epic orchestration templates define inner_workflow and wave_pattern; validate the project execution plan separately.'
      );
  ...
  validatePhaseSequence(sequence) {
    ...
    for (let i = 0; i < sequence.length; i++) {
      ...
      // Workflow families use phase headers and meta/control entries to make
      // the execution plan readable. They are structural, not executable
      // agent steps, so they do not require an agent or action contract.
      const isPhaseHeader = step.phase !== undefined && step.name && !step.agent;
      const isMetaEntry = Boolean(step.meta) && !step.agent;
      if (isPhaseHeader || isMetaEntry) {
        continue;
      }

      const hasAction = step.creates || step.updates || step.validates || step.action ||
        step.task || step.script || step.workflow || step.run || step.outputs ||
        step.condition_check || step.commands;

      if (!step.agent && !hasAction) {
        result.warnings.push({
    ...
    for (const step of sequence) {
      if (step.agent) {
        ...
        for (const a of agentNames) {
          // Workflow files commonly use @agent aliases. Runtime expressions
          // such as ${story.executor} are resolved by the orchestrator and
          // must not be checked as literal filenames.
          if (!a || a.includes('${') || a.includes('{') || a.includes('}')) continue;
          agents.add(a.replace(/^@/, ''));
        }
      }
    }

    // Skip meta-agent names
    const metaAgents = new Set(['various', 'system']);
      ...
      if (step.requires) {
        ...
        if (typeof required === 'string' && !required.startsWith('all_') && !required.startsWith('sharded_')) {
          const priorStepIds = new Set(
            sequence.slice(0, i).flatMap((priorStep) => [priorStep.id, priorStep.step].filter(Boolean))
          );
          const isWorkflowDependency = priorStepIds.has(required);
          const isExternalInput = required.startsWith('existing_') || required.startsWith('environment_');
          if (!createdArtifacts.has(required) && !isWorkflowDependency && !isExternalInput) {
            result.warnings.push({
  ...
  validateHandoffPrompts(workflow, phaseSequence = null) {
    ...
    const automatedWorkflowTypes = new Set(['loop', 'pipeline', 'orchestration']);
    if (transitionCount > 0 && !wf.handoff_prompts && !automatedWorkflowTypes.has(wf.type)) {
      result.warnings.push({
  ...
  validateMermaidDiagram(workflow) {
    ...
    // ASCII diagrams and other descriptive flow text are valid. Apply the
    // Mermaid-specific checks only when the document actually declares a
    // Mermaid block or graph/flowchart expression.
    const isMermaid = /```\s*mermaid\b/i.test(diagram) || /\b(?:graph|flowchart)\s+(?:TD|TB|BT|RL|LR)\b/i.test(diagram);
    if (!isMermaid) return result;
