...
async function validateDependencies(agents) {
  ...
  // AIOX keeps product-facing artifacts outside development/ while legacy
  // agent definitions still use the short dependency type names. Search the
  // canonical locations first, then retain legacy locations for compatibility.
  const depDirs = {
    tasks: [TASKS_DIR],
    templates: [path.join(ROOT_DIR, 'product', 'templates'), TEMPLATES_DIR],
    checklists: [path.join(ROOT_DIR, 'product', 'checklists'), CHECKLISTS_DIR],
    data: [path.join(ROOT_DIR, 'product', 'data'), path.join(ROOT_DIR, 'data'), DATA_DIR],
    utils: [path.join(ROOT_DIR, 'infrastructure', 'scripts'), path.join(ROOT_DIR, 'core', 'utils'), path.join(ROOT_DIR, 'core', 'execution'), path.join(ROOT_DIR, 'core', 'memory'), path.join(ROOT_DIR, 'scripts'), path.join(ROOT_DIR, 'utils'), path.join(ROOT_DIR, 'development', 'scripts'), UTILS_DIR],
    workflows: [WORKFLOWS_DIR],
    scripts: [path.join(ROOT_DIR, 'infrastructure', 'scripts'), path.join(ROOT_DIR, 'core', 'execution'), path.join(ROOT_DIR, 'core', 'memory'), SCRIPTS_DIR],
    schemas: [path.join(ROOT_DIR, 'schemas'), path.join(ROOT_DIR, 'infrastructure', 'schemas')],
  };
  ...
  for (const agent of agents) {
    ...
    for (const [depType, depList] of Object.entries(deps)) {
      ...
      const depSearchDirs = depDirs[depType];
      if (!depSearchDirs) {
        warnings.push({
      ...
      for (const depFile of depList) {
        const depPaths = depSearchDirs.flatMap((depDir) => {
          const exact = path.join(depDir, depFile);
          if (path.extname(depFile)) return [exact];
          return [exact, `${exact}.js`, `${exact}.md`, `${exact}.yaml`, `${exact}.yml`];
        });
        const existingPath = await depPaths.reduce(async (found, depPath) => {
          const resolved = await found;
          return resolved || (await fileExists(depPath) ? depPath : null);
        ...
        if (!existingPath) {
          ...
          warnings.push({
            ...
            depType,
            depFile,
            expectedPath: depPaths.join(' or '),
            message: `Missing dependency: @${agent.id} → ${depType}/${depFile}`,
            suggestion: `Create the file in one of the canonical dependency locations or remove it from the agent dependencies.`,
          });
        }
      }
    }
