...
const SCAN_SOURCES = [
  ...
  {
    pattern: '.aiox-core/data/**/*.md',
    category: 'data',
    taskFormat: 'TASK-FORMAT-V1',
    subcategoryExtractor: () => 'knowledge',
  },
  {
    pattern: '.aiox-core/data/**/*.yaml',
    category: 'data',
    taskFormat: 'TASK-FORMAT-V1',
  ...
  for (const source of SCAN_SOURCES) {
    ...
    try {
      ...
      for (const file of files) {
        ...
        // Ensure unique IDs even when different directories contain the same basename
        const baseId = worker.id;
        let duplicateIndex = 1;
        while (seenIds.has(worker.id)) {
          worker.id = `${baseId}-${source.category}-${duplicateIndex}`;
          duplicateIndex += 1;
        }
