...
async function run(context) {
  ...
  // Boundary protection is intentionally disabled for framework-maintainer
  // checkouts. In that mode, requiring project deny rules is contradictory to
  // the configured policy and produces a false warning.
  const configPath = path.join(context.projectRoot, '.aiox-core', 'core-config.yaml');
  if (fs.existsSync(configPath)) {
    try {
      const config = yaml.load(fs.readFileSync(configPath, 'utf8'));
      if (config?.boundary?.frameworkProtection === false) {
        return {
          check: name,
          status: 'PASS',
          message: 'Boundary protection disabled by core-config.yaml',
          fixCommand: null,
        };
      }
    } catch {
      // Keep the existing validation path; malformed configuration is reported
      // by the dedicated config checks.
    }
  }
